...
> **이 앱은 오디오비주얼 도구가 아니다.** TouchDesigner CHOP Reference 의 *방식론* — 실시간 채널을 오브젝트 속성에 바인딩 — 만 차용한 **데이터 참조형 그래프 WebAR 에디터**다. 시간/타깃/얼굴/포인터/랜덤/오디오 등 모든 실시간 신호가 동등한 시민이며, 오디오는 그중 하나의 입력 source 일 뿐이다.

이 에디터는 단순히 "클릭하면 실행되는 이벤트 노드"에 머물면 안 된다. 미디어아트 작가에게 더 중요한 것은 시간, 타깃 거리/회전, 얼굴, 포인터, 랜덤, 오디오 같은 실시간 데이터를 AR 오브젝트의 속성에 연결하는 방식이다.

따라서 TouchDesigner의 CHOP Reference 개념을 참고해 **Signal Binding** 시스템을 핵심 설계 원칙으로 둔다.

기본 아이디어 — 어떤 source 든 어떤 속성에든 매핑 가능:

```txt
time.sin            -> text.opacity
target.confidence   -> object.material.emissiveIntensity
face.yaw            -> particleSystem.spread
pointer.x           -> object.position.x
random.noise        -> object.rotation.z
audio.low           -> object.scale
```
...
| Signal Graph | 매 프레임 변하는 값을 속성에 연결 | 시간, 타깃 변환, 얼굴 회전, 포인터, 랜덤, 오디오 등 |
...
```txt
[time.sin] -> [Map -1..1 to 0..1] -> [text.opacity]
```

```txt
[target.confidence] -> [Smooth 0.2] -> [object.material.emissiveIntensity]
```

```txt
[face.yaw] -> [Clamp -45..45] -> [particleSystem.spread]
```

```txt
[audio.low] -> [Map 0..1 to 0.8..2.5] -> [object.scale]
```

이 구분이 중요하다. 데이터 반응형 · 인터랙티브 · 시간 기반 · 퍼포먼스 작업은 대부분 Event Graph 보다 Signal Graph 가 더 중요하다.
...
> 모든 source 는 동등한 시민이다. 아래 순서는 *작가가 가장 자주 쓸 것으로 예상하는 순서* 일 뿐, 우선순위가 아니다.

시간 (가장 단순, 어떤 작품에든 사용 가능):
...
오디오 (마이크 / 외부 사운드 분석 — 다른 source 와 동등):
...
- `audio.peak`
- `audio.beat`
- `audio.spectrum.N` (dot-case `^audio\.spectrum\.\d+$`)
  ...
  "bindings": [
    {
      "id": "binding-001",
      "source": "time.sin",
      "target": "objects.poster-text.opacity",
      "map": {
        "in": [-1, 1],
        "out": [0.3, 1.0],
        "clamp": true,
    ...
    },
    {
      ...
      "target": "objects.poster-text.material.emissiveIntensity",
      "map": {
        "in": [0, 1],
        "out": [0.0, 4.0],
        "clamp": true,
    ...
    },
    {
      "id": "binding-003",
      "source": "audio.low",
      "target": "objects.poster-text.scale",
      "map": {
        "in": [0, 1],
        "out": [0.8, 2.5],
        "clamp": true,
...
| time.sin | 부드럽게 반복 |
| time.seconds | 흐른 시간 |
| target.visible | 타깃 보임/숨김 |
| target.confidence | 인식 확신도 |
| pointer.x / pointer.y | 손가락 위치 |
| pointer.down | 누르고 있음 |
| face.yaw | 고개를 좌우로 돌림 |
| face.pitch | 고개를 위아래로 움직임 |
| random.noise | 부드러운 무작위 흔들림 |
| audio.low | 음악의 낮은 소리 |
| audio.mid | 음악의 중간 소리 |
